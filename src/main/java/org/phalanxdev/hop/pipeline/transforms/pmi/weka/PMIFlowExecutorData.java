 ...
 * @version $Revision: $
 */
public class PMIFlowExecutorData extends BaseTransformData implements ITransformData {
   ...
   */
  protected void setOutputRowMeta( IRowMeta rmi ) {
    m_outputRowMeta = rmi;
  }
   ...
   */
  protected IRowMeta getOutputRowMeta() {
    ...
    for ( int i = 0; i < m_injectFieldIndexes.length; i++ ) {
      if ( m_injectFieldIndexes[i] >= 0 ) {
        if ( m_injectArffMetas[i].getArffType() == ArffMeta.NOMINAL ) {
          ...
          if ( org.apache.hop.core.util.Utils.isEmpty( m_injectArffMetas[i].getNominalVals() ) ) {
  ...
  public static Flow getFlowFromFileVFS( String fileName, IVariables vars, Environment env ) throws Exception {
    if ( !org.apache.hop.core.util.Utils.isEmpty( fileName ) ) {
      ...
      FileObject fo = HopVfs.getFileObject( fileName );
  ...
  protected void validateInputStep( String targetStepName, String inputConnName, boolean streaming, IVariables vars )
      throws HopException {

    if ( org.apache.hop.core.util.Utils.isEmpty( inputConnName ) ) {
      throw new HopException(
          BaseMessages.getString( PMIFlowExecutorMeta.PKG, "KnowledgeFlowData.Error.NoInputConnectionSpecified" ) );
    }

    if ( org.apache.hop.core.util.Utils.isEmpty( targetStepName ) ) {
      throw new HopException(
  ...
  protected static StepManagerImpl validateOutputStep( Flow flowToUse, String outputStepName, String outputConnName,
      StepOutputListener outListener, IVariables vars, ILogChannel log ) throws HopException {

    if ( org.apache.hop.core.util.Utils.isEmpty( outputStepName ) || org.apache.hop.core.util.Utils
        .isEmpty( outputConnName ) ) {
      throw new HopTransformException( BaseMessages.getString( PMIFlowExecutorMeta.PKG,
    ...
    if ( outListener != null ) {
      if ( org.apache.hop.core.util.Utils.isEmpty( outputConnName ) ) {
        throw new HopException(
   ...
   * @throws HopException if the conversion can't be performed
   */
  protected Instance constructInstance( IRowMeta inputRowMeta, Object[] row, Instances header, boolean batch )
      throws HopException {
    ...
    for ( int i = 0; i < m_injectArffMetas.length; i++ ) {
      ...
      if ( tempField != null ) {
        ...
        IValueMeta vmi = inputRowMeta.getValueMeta( m_injectFieldIndexes[i] );
   ...
   * @throws HopException if there is a problem during the conversion
   */
  protected Instances reservoirToInstances( IRowMeta inputRowMeta ) throws HopException {
   ...
   * @throws HopException if there is a problem constructing the header
   */
  private Instances createHeader( IRowMeta inputRowMeta ) throws HopException {
   ...
   */
  private List<String> setupNominalVals( int index, IValueMeta fieldMeta ) throws HopException {
    ...
    for ( Object val : keySet ) {
      ...
      if ( org.apache.hop.core.util.Utils.isEmpty( m_injectArffMetas[index].getNominalVals() ) ) {
        String sval = fieldMeta.getString( val );
        if ( !org.apache.hop.core.util.Utils.isEmpty( sval ) ) {
          sorted.add( sval );
        }
      } else {
  ...
  protected void injectDataBatch( ExecutionFinishedCallback finishedCallback, IRowMeta inputMeta,
      PMIFlowExecutorMeta kfMeta, IVariables vars ) throws HopException, WekaException {
  ...
  protected void flushStreamingBuffer( IRowMeta inputMeta, Object[] inputRow ) throws HopException {
  ...
  protected void processRow( Object[] inputRow, IRowMeta inputMeta ) throws Exception {
  ...
  protected void buildAndValidateInjectMapping( IRowMeta inputMeta ) throws HopException {
  ...
  public static class LogAdapter extends PrintStream implements Serializable, Logger {
    ...
    public LogAdapter( ILogChannel wrapped ) {
      super( System.out );
      m_log = wrapped;
    }
