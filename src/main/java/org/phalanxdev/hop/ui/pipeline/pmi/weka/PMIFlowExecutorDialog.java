 ...
 */
public class PMIFlowExecutorDialog extends BaseTransformDialog implements ITransformDialog {
  ...
  @Override public String open() {
    ...
    wlTransformName.setLayoutData( fdlStepname );
    ...
    m_fdTabFolder.top = new FormAttachment( wTransformName, margin );
  ...
  private void ok() {
    ...
    // Filename stuff
    if ( !Utils.isEmpty( m_wFilename.getText() ) && !m_currentMeta
    ...
    if ( Utils.isEmpty( m_currentMeta.getSerializedFlowFileName() ) && m_currentMeta
        .getStoreFlowInStepMetaData() ) {
      try {
  ...
  private void setUpFileTab( final int middle, final int margin, ModifyListener lsMod ) {
    ...
    m_wOutputStepComboBox.addModifyListener( new ModifyListener() {
      @Override public void modifyText( ModifyEvent e ) {
        m_currentMeta.setChanged();
        if ( !Utils.isEmpty( m_wOutputStepComboBox.getText() ) ) {
    ...
    m_wbKFeditor.addSelectionListener( new SelectionAdapter() {
      @Override public void widgetSelected( SelectionEvent e ) {
        ...
        if ( Utils.isEmpty( m_wFilename.getText() ) || m_storeFlowInStepMetaData
            .getSelection() ) {
          m_wbGetEditorChanges.setEnabled( true );
        }
      }
  ...
  private void setupInjectConnNames( String stepName ) {
    if ( m_currentFlow != null ) {
      ...
      if ( !Utils.isEmpty( old ) && m_wInjectEventComboBox.indexOf( old ) >= 0 ) {
  ...
  private void setupInjectStepNames() {
    ...
    if ( !Utils.isEmpty( old ) && m_wInjectStepComboBox.indexOf( old ) >= 0 ) {
  ...
  private void setupOutputStepNames() {
    ...
    // restore the old value (if still valid)
    if ( !Utils.isEmpty( old ) && m_wOutputStepComboBox.indexOf( old ) >= 0 ) {
  ...
  private void setupOutputConnNames( String stepName ) {
    ...
    // restore the old value (if still valid)
    if ( !Utils.isEmpty( old ) && m_wOutputEventComboBox.indexOf( old ) >= 0 ) {
    ...
    if ( fields != null ) {
      ...
      for ( int i = 0; i < fields.length; i++ ) {
        if ( fields[i] != null ) {
          ...
          } else if ( !Utils.isEmpty( fields[i].getNominalVals() ) ) {
  ...
  private ArffMeta[] setupArffMetas() {
    ...
    TransformMeta thisStepMeta = pipelineMeta.findTransform( transformName );
  ...
  private boolean loadFlow() {
    ...
    if ( Utils.isEmpty( filename ) && !m_wbKFeditor.isEnabled() ) {
      m_wbGetEditorChanges.setEnabled( true );
    }
