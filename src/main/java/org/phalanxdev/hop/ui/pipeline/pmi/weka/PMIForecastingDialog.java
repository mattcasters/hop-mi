...
import org.phalanxdev.hop.pipeline.transforms.pmi.weka.PMIForecastingData;
import org.phalanxdev.hop.pipeline.transforms.pmi.weka.PMIForecastingMeta;
import org.phalanxdev.hop.pipeline.transforms.pmi.weka.WekaForecastingModel;
 ...
 * @version $Revision$
 */
public class PMIForecastingDialog extends BaseTransformDialog implements ITransformDialog {
  ...
  @Override public String open() {
    ...
    formLayout.marginWidth = org.apache.hop.core.Const.FORM_MARGIN;
    formLayout.marginHeight = org.apache.hop.core.Const.FORM_MARGIN;

    shell.setLayout( formLayout );
    shell.setText( BaseMessages.getString( PMIForecastingMeta.PKG, "PMIForecastingDialog.Shell.Title" ) );

    int middle = 50;
    int margin = org.apache.hop.core.Const.MARGIN;
    ...
    m_wStepname.setText( transformName );
      ...
      if ( stepMeta != null ) {
        ...
        for ( int i = 0; i < header.numAttributes(); i++ ) {
          ...
          if ( mappings[i] == PMIForecastingData.NO_MATCH ) {
            ...
            IValueMeta tempField = rowM.getValueMeta( mappings[i] );
  ...
  public void getData() {
    ...
    if ( !org.apache.hop.core.util.Utils.isEmpty( m_currentMeta.getSavedForecasterFileName() ) ) {
  ...
  private void ok() {
    if ( org.apache.hop.core.util.Utils.isEmpty( m_wStepname.getText() ) ) {
      return;
    }

    transformName = m_wStepname.getText(); // return value

    if ( !org.apache.hop.core.util.Utils.isEmpty( m_wFilename.getText() ) ) {
      m_currentMeta.setSerializedModelFileName( m_wFilename.getText() );
    } else {
      m_currentMeta.setSerializedModelFileName( null );
    }

    if ( !org.apache.hop.core.util.Utils.isEmpty( m_numStepsText.getText() ) ) {
      try {
    ...
    if ( !org.apache.hop.core.util.Utils.isEmpty( m_artificialTimeOffsetText.getText() ) ) {
      try {
    ...
    if ( !org.apache.hop.core.util.Utils.isEmpty( m_saveForecasterField.getText() ) ) {
